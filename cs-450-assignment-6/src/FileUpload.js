  ...
  handleFileSubmit = (e) => {
    ...
    if (this.state.file) {
      ...
      reader.onload = (e) => {
        ...
        d3.csv(url)
          .then((data) => {
            // Use d3.csv to parse the CSV data (asynchronously)
            const formattedData = data.map((d) => ({
              // Format the data
              Date: new Date(d.Date),
              "GPT-4": Number(d["GPT-4"]),
              "Gemini": Number(d["Gemini"]),
              "PaLM-2": Number(d["PaLM-2"]),
              Claude: Number(d.Claude),
              "LLaMA-3.1": Number(d["LLaMA-3.1"]),
            }));
